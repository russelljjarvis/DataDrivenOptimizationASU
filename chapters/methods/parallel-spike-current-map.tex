...
\begin{figure}    
  ...
  \includegraphics[width=0.9\linewidth]{{figures/rheobase_algorithm.png}}
    \caption[Parallel rheobase algorithm]{I developed a generic algorithm which found the minimal current injection value that would cause only one spike (rheobase). While the would normal proceed by brute force or binary search, I implemented an N-ary search that used multiple parallel simulations to more rapidly identify the rheobase. This lead to significant speed-ups for testing and optimization in all models.}
    \label{fig:rheobase}
  \end{center}
\end{figure} 

...
Conflicts between Rheobase value and other static electrical measurements were observed in NeuroElectro and Allen measurements. When optimizing against literature derived averaged measurements, and Allen Brain single cell observations, it was common to experience a conflicted ability for a given model to satisfy all constraints simultaneously. 

Models seemed to have particular difficulty in recapitulating an accurate fit for rheobase, while simultaneously satisfying the all of the other fitness criteria: time constant, input resistance, capacitance, resting membrane potential and FISlope. 
...
When comparing generic single compartment models, to the BBP neocortical layer 5 neuron, one can see that there are often conflicts between Rheobase, FISlope and Time Constant mearurements. As a pattern the conductance based models sampled in this work seemed to have trouble satisfying these three measurements at the same time.
